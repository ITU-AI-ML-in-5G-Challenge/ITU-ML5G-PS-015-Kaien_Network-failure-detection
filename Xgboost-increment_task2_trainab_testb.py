...
fx=open('csvFile/abc1109.csv','rb')
df = pd.read_csv(fx,index_col=None) 
fx.close

train = df[(df['traintest']==0) & (df['abc']=='a')]  #if borrow task_1 model a
test = df[(df['traintest']==1) & (df['abc']=='b')] #a,c

y_train = train['label']#.values  #label60percent
...
y_test = test['label']
test = test.drop(['label','labelnew','label2','abc','traintest','datetime','name'#,'operstatus1onehot','operstatus2onehot','operstatus3onehot','minutestatusconditiononehot','minutestatusconditiononehot','memorystatusonehot'],axis=1)
                    ],axis=1)
...
clf0 = xgb.train(params, dtrain, num_boost_round=50)
